...
export class QuoteGenerator {
  ...
  addProduct() {
    ...
    product.cantidad = Number.parseInt(product.cantidad);
    product.precio = Number.parseFloat(product.precio);
  ...
  async generateQuoteImage() {
    if (!this.state.hasProducts()) {
      // notify user
      Validations.notyf.error(
        "No hay productos en la cotización. Agrega al menos uno para continuar.",
      );
