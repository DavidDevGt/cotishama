...
router.get("/summary", requireRole("ADMIN", "OPERATOR"), async (c) => {
  try {
    ...
    const summary = {
      ...
      totalValue: quotes.reduce((sum, q) => sum + Number.parseFloat(q.total), 0),
      averageValue:
        quotes.length > 0
          ? quotes.reduce((sum, q) => sum + Number.parseFloat(q.total), 0) / quotes.length
          : 0,
    };
...
router.get("/metrics", requireRole("ADMIN"), async (c) => {
  try {
    ...
    const metrics = {
      ...
      medianQuoteValue:
        quotes.length > 0
          ? quotes.sort((a, b) => Number.parseFloat(a.total) - Number.parseFloat(b.total))[
