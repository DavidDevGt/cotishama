...
import { ConflictError, NotFoundError, ValidationError } from '../../../apps/backend/src/types/errors';
...
describe('QuoteService', () => {
  ...
  describe('#createQuote', () => {
    ...
    it('should calculate total correctly', async () => {
      ...
      expect(parseFloat(result.subtotal)).toBe(2000);
    });
    ...
    it('should apply discount to line items', async () => {
      ...
      expect(parseFloat(result.subtotal)).toBe(900);
  ...
  describe('#changeStatus', () => {
    ...
    it('should handle all valid statuses', async () => {
      ...
      for (const status of statuses) {
        if (status !== 'DRAFT') { // Start from draft for each
  ...
  describe('complex scenarios', () => {
    it('should handle quote with multiple products', async () => {
      ...
      expect(parseFloat(result.subtotal)).toBe(1400); // 500 + 600 + 300
    });
