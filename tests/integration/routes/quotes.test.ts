...
describe("Quote Routes - Integration", () => {
  ...
  describe("GET /api/v1/quotes", () => {
    ...
    it("should filter by status", async () => {
      const response = await app.request("/api/v1/quotes?status=DRAFT", {
        method: "GET",
        headers: { Authorization: `Bearer ${authToken}` },
      });
    ...
    it("should respect pagination", async () => {
      const response = await app.request("/api/v1/quotes?limit=1", {
        method: "GET",
        headers: { Authorization: `Bearer ${authToken}` },
      });
  ...
  describe("PATCH /api/v1/quotes/:id/status", () => {
    ...
    it("should change quote status", async () => {
      const response = await app.request(`/api/v1/quotes/${quoteId}/status`, {
        method: "PATCH",
  ...
  describe("complex scenarios", () => {
    it("should handle complete quote workflow", async () => {
      ...
      // Send
      const sendResponse = await app.request(`/api/v1/quotes/${quoteId}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status: "SENT" }),
        headers: {
          "Content-Type": "application/json",
      ...
      // Accept
      const acceptResponse = await app.request(`/api/v1/quotes/${quoteId}/status`, {
        method: "PATCH",
